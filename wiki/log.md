...
- Summary: wiki/synthesis/2026-09-02/conference-digest.md — Conference & arXiv daily digest organized by venue/category, focused on the fresh Sep-2026 submission wave (IDs `2609.0xxxx`, submitted Aug 31–Sep 2) and conference acceptances (EMNLP 2026, ICML 2026, IEEE ICDM 2026, 35th ACM/SIGIR-adjacent, ASE 2026 SRC, SOSP AgenticOS 2026). Every arXiv ID here is grep-verified absent from `wiki/` (dedup vs same-day arxiv-daily / arxiv-ai-search / arxiv-paper-check).
- **45+ papers across 7 categories**: ① LLM Arch/MoE/Efficient Inference (Instella-MoE AMD fully-open 16B/2.8B MoE; PCoMoE fine-grained path composition EMNLP'26; SMELT compute-matched looped-MoE scaling laws; DynaNDE near-data expert scheduling; residual sparsification via output importance EMNLP'26; Faster Than Flash attention-sparsity ICDM long-context ICML'26; CacheBridge cross-model KV; QTEA ternary; OCGQuant NVFP4; HBQ quantization); ② LLM Post-Training/RLVR/Alignment (Where the Verifier Fails verifier category audit; From Base Rollouts to RL Reasoning EMNLP'26 Findings; ARISE-RL; Post-Training Science for SFT; mid-training KD favors reasoning; Behaviorally Effective LoRA Writes Are Sparse; Enoki hallucination); ③ Industrial Rec/Ads/Sequential (ReST Rec-native ranking +1.31% AUC/+11.93% revenue; TGR Tencent unified generation+reasoning cold-start Hit@1 +477.8%; CoGR co-evolving generative retriever; TS-SSM Findings EMNLP'26; ICDM'26 mobile-game cross-source user rep; 35th ACM conversational-recommendation protocol study; HypReflect; counterfactual evidence rec); ④ World Models/Games/Embodied (HyperWorld; IMPACT attention-as-interaction-map; Streaming4D; GUI-CC Findings EMNLP'26; ZimaBlue; NashDreamer); ⑤ Code/Agent Engineering/Systems (CUDA-Harness; WiseSpec ASE'26 SRC; trajectory-aware SWE benchmarking; Irreversibility Budget SOSP AgenticOS); ⑥ Generative/Diffusion/VL (ReNFT; diffusion-secretly-calculates-attentions; SinkPruner Findings EMNLP'26; diffusion-as-curriculum); ⑦ Benchmarks (RPCBench; WorldBench; MUSES).
- Cross-cutting themes: MoE → fine-grained path composition + near-data scheduling + hardware-aware low-bit quant; industrial recommendation → unified generation + reasoning (TGR/ReST/CoGR); RLVR enters honesty/audit era (verifier category-level failures, rollout re-distribution vs capability creation); world models → executable/interactive (hypergraph serialization, attention-as-interaction, streaming 4D); post-training becomes a science (SFT recipe, KD stage effects, sparse LoRA writes).
- Updated: wiki/index.md (Synthesis table — new conference-digest 2026-09-02 entry at top of the 09-02 batch)
