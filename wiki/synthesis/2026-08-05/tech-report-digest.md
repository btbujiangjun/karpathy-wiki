...
created: 2026-08-05
updated: 2026-08-05
sources: [tech-report-digest-2026-08-04.md]
...
> 更新日期：2026-08-05（每日更新；今日重点：NVIDIA Nemotron 3 Super 技术报告、OpenAI GPT-5.6 降价、Gemini Robotics ER 2 / On-Device 2 模型卡、Amazon Nova 战略收缩与 FMR 重组、MiniMax H3 开源权重、Intern-S2-Preview-397B 确认、Mistral Shieldstral、GPT-5.7 / GLM-5.5 / Phi-5 传闻核实）
...
> ⚠️ 传闻更新：WinCentral 2026-07-30 报道 **GPT-5.7**（8 月发布，新的 pretraining foundation，~10T tokens），GPT-6 或推迟至 9 月；另据 The Information 报道 OpenAI 内部代号 **Astra** 的新模型家族（面向长期运行任务，已解决 10 道长期未解数学题），命名可能在 GPT-5.7 / GPT-6 之间未定。均为单源/记者爆料，截至 08-05 未获官方确认，不写入正式条目。

---

## 3. Meta

### 3.1 Muse Spark Safety & Preparedness Report（今日补充评估细节）
...
### 4.1 Gemini Robotics ER 2 / On-Device 2 Model Card（今日新增核实）
...
> 今日核实：Gemini 4 预训练已启动（Gemini family update 2026-07）；Gemini 3.5 Pro 仍延迟未发布。
...
### 7.1 Qwen3.8-Max（08-04 已收录；今日补充定价与 27B 版本）
...
> 观察（noze.it 评论）："下周开源"仅承诺，缺具体日期 + license + model card；Qwen3.8-27B 才是普通企业可自托管的关键 checkpoint。此前 3 天 Moonshot 的 Kimi K3 已给出明确权重日期。
...
> ⚠️ 传闻更新：**Phi-5**（2026-05-06 起有报道，约 16B 参数，MMLU 86.7%、HumanEval 82.1%，声称匹配 GPT-4 级别）——single-source（GogoAI），官方 MSR 页面仍以 Phi-4-reasoning-vision-15B 为最新技术报告，**未确认，不写入正式条目**。
...
> 今日核实：截至 2026-08-05 仍无正式技术报告（"later this summer" 预期未兑现）；2026-07-28 Siri Expressive Voices 博客为最新 ML 研究输出。

---

## 10. NVIDIA

### 10.1 Nemotron 3 Super Technical Report（今日新增核实——Nemotron 3 家族补齐）
...
> Nemotron 3 三档（Nano 08-04 / Super 今日 / Ultra 08-03）技术报告现已全部收录，构成完整家族：混合 Mamba-Attention + LatentMoE + NVFP4 预训练 + 1M ctx + 开源。
...
> ⚠️ 传闻更新：Musk（2026-07-28）确认 **Grok 4.6 于 2026-08-07 发布**（1.5T 参数 V9 基座、改进 SFT/RL），**Grok 4.7**（2.1T）随后数周发布。较此前"Grok 5 于 Colossus 2 训练 Q3"的说法更具体——Grok 5 口径需继续核实。均未正式发布，不作为正式条目。

---

## 12. Amazon

### 12.1 Nova 系列收缩 + Frontier Model Research（今日新增核实——重大战略转向）
...
### 13.1 Seed2.0 系列（今日补充 Omni-Modal 细节）
...
> ⚠️ **GLM-5.5** 截至 2026-08-03（AI Release Tracker 口径）仍未发布：JPMorgan 研报（Reuters/CGTN 转载）称可能 2026-08 发布，1T+ 参数、1M ctx。单源传闻，不写入正式条目；GLM-5.2 仍为当前确认旗舰。
...
> 今日核实：无 8 月新报告；K3 权重已按承诺日期发布（对照 Qwen3.8-Max 的"下周"）。

---

## 16. InternLM（上海 AI 实验室）

### 16.1 Intern-S2-Preview 系列（今日确认——08-04 待核实项落地）
...
> 08-04 digest 的"🔍 新发现 Intern-S2 Collections（待核实）"已确认：35B 与 397B 均已发布并可下载。
...
- **Step 3.7 Flash**（当前旗舰）：198B 总 / 11B 激活稀疏 MoE；原生多模态；256K ctx；low/medium/high 三级 reasoning level。
...
### 20.1 MiniMax H3 开源（今日更新——"open weights 计划中"已落地）
...
- **传闻需谨慎（未确认不入正式条目）**：GPT-5.7/Astra（The Information + WinCentral）、GLM-5.5（JPMorgan 8 月）、Phi-5（GogoAI 单源）、Grok 4.6/4.7（Musk 口头时间表）。
