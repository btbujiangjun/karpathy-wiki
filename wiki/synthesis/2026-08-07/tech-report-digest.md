...
created: 2026-08-07
updated: 2026-08-07
sources: [tech-report-digest-2026-08-06.md]
...
> 更新日期：2026-08-07（每日更新；今日重点：**Grok 4.6 观察日（今日 08-07）**——Musk 预告 1.5T 参数 V9 基座、大幅升级 SFT/RL，第三方基准站 kie.ai 称已上线，但 xAI 官方模型目录仍仅列 grok-4.5，**待官方确认**；**Qwen3.8-Max 开源权重窗口**（08-03 承诺"下周"≈08-10 前后，仍缺具体日期/license/model card）；OpenAI **GPT-5.7 泄漏复核**（8 月发布、新 pretraining foundation ~10T tokens、GPT-6 推迟 9 月，未确认）；Anthropic 新增 **Claude Opus 5 System Card**（07-24 已收录）+ Sonnet 5 System Card 更新至 07-10；**GLM-5.5 传闻**（JPMorgan 8 月）、**Kimi Slides 演示**（08-03）、**Kimi K4 Blackwell 训练传闻**；Microsoft **Phi-5 仍无官方技术报告**；Meta / Google / NVIDIA / Mistral / Amazon / ByteDance / MiniMax 等均无 8 月新报告）
...
> ⚠️ 传闻更新（今日复核）：WinCentral 2026-07-30 报道 **GPT-5.7**（8 月发布，新的 pretraining foundation，约 10T tokens 规模训练，更强推理与 agent 能力），GPT-6 或推迟至 9 月；另据 The Information 报道 OpenAI 内部代号 **Astra** 的新模型家族（面向长期运行任务，已解决 10 道长期未解数学题），命名可能在 GPT-5.7 / GPT-6 之间未定。截至 08-07 仍为记者爆料，未获官方确认，不写入正式条目。
...
> 今日状态（开源窗口倒计时）：08-03 承诺"下周"开源 → 预计 **08-10 前后**（明日即 8 日进入窗口）；截至 08-07 仍无具体日期 + license + model card。对照 Moonshot Kimi K3 的明确权重日期（07-27 已兑现），Qwen 缺最后三项细节受评论界持续质疑。**08-08~08-14 为权重观察窗口**。
...
> ⚠️ 传闻更新（今日核实）：**Phi-5** 截至 08-07 仍无官方技术报告——唯一新增为 Inference Index 目录条目（2026-01-08，128K ctx，小模型 champion）；此前的 16B / MMLU 86.7% 报道为 single-source（GogoAI），官方 MSR 页面仍以 Phi-4-reasoning-vision-15B 为最新技术报告，**未确认，不写入正式条目**。
...
> ⚠️ **Grok 4.6 观察日（今日 08-07）**：Musk（2026-07-28；Economic Times 07-29 报道）确认 **Grok 4.6 预计 2026-08-07 发布**——1.5T 参数 V9 基座（与 4.5 相同）、重点在**大幅升级的 SFT + RL 后训练**而非新架构；性能目标直指 Kimi K3 与 Claude Opus 4.8；保持 4.5 的吞吐与 token 效率（~80 tps）。**Grok 4.7**（2.1T）随后数周发布。第三方基准聚合站 kie.ai 的 Grok 4.6 页面显示**已上线（2026-08-07）**，但 xAI 官方文档 models 目录截至搜索时**仍仅列 grok-4.5**（$2/$6，500K ctx，知识截止 2026-02-01）。→ 发布仍属"第三方称已上线、官方待确认"，一旦官方文档更新再升级为正式条目。
...
> ⚠️ **GLM-5.5** 截至 08-07 仍未发布：JPMorgan 研报（Reuters/CGTN 转载）称可能 2026-08 发布，1T+ 参数（基于 GLM-5.2 744B 升级）、1M ctx。单源传闻，不写入正式条目；GLM-5.2 仍为当前确认旗舰。
...
> 今日新增：**Kimi Slides 演示**（08-03，K3 生成式 PPT 生成）；**Kimi K4** 训练阶段传闻（AI Weekly 07-28：Moonshot 寻求更多 NVIDIA Blackwell 芯片）——未发布，不入正式条目。K3 技术报告（07-27）仍为最新；K3 权重已按承诺日期发布（对照 Qwen3.8-Max 的"下周"）。
...
- **Grok 4.6 = 今日观察点，Qwen3.8-Max 开源窗口 = 明日观察点**：Musk 口头时间表（08-07）今日兑现与否待官方确认（第三方 kie.ai 显示已上线，xAI 文档未更新）；Qwen3.8-Max 权重"下周"（08-08~08-14 窗口开启）——两大"承诺制发布"进入验收期。
- **8 月上旬密集发布窗口仍延续**：Grok 4.6（今日）+ 传闻中 GPT-5.7（8 月）+ Qwen3.8-Max 开源——若兑现，将与 Nemotron 3 家族、Kimi K3 构成 8 月第二波"旗舰对决"。
...
- **传闻需谨慎（未确认不入正式条目）**：GPT-5.7/Astra（The Information + WinCentral）、GLM-5.5（JPMorgan 8 月）、Phi-5（仅 Inference Index 目录条目）、Grok 4.6/4.7（Musk 口头时间表，08-07 观察、第三方称已上线）、Kimi K4（Blackwell 训练传闻）。
