...
created: 2026-08-15
updated: 2026-08-15
sources: [tech-report-digest-2026-08-14.md]
...
> 更新日期：2026-08-15（每日更新；今日重点：**Qwen3.8-27B 正式开源（08-14）**——27B dense 原生多模态（image+video）、**Apache 2.0**、262K native ctx（YaRN 可扩 1M）、Hybrid Gated DeltaNet + Gated Attention（64 层 = 16 组重复块）、DeepSWE v1.1 **42.2**（vs Qwen3.6-27B 的 13.3）/Terminal-Bench 2.1 **73.0**/SWE-bench Pro **61.7**/OSWorld-Verified **84.3**/LiveCodeBench v6 **90.3**，与 2.4T 旗舰权重形成鲜明"双轨"对照——**小的是宽松+多模态（Apache 2.0 + native vision），大的是定制 license + text-only + thinking 强制**；**智谱 GLM-5.3 正式发布（08-14）**——**"基座不变、后训练提智"**：与 GLM-5.2 相同 743B 基座，全部提升来自后训练 Scaling（数十倍长程任务环境 + IndexShare + SAO + 新一代 Slime 框架），Terminal-Bench 3.0 **4.6→28.3 开源第一**、DeepSWE v1.1 46.2→66.9、编程体感较 5.2 +50%，网络安全白盒代码审查/漏洞发现持平 Mythos 5（真实代码库累计识别 2436 个安全漏洞），权重两周后开源；修正 08-14 "GLM-5.3 为传闻"记录（此前景观财经 >1T 传闻错误，实际为 743B 同基座）；**Meta Llama 4 405B 开放权重持续未兑现（第 4 天）**；**Qwen3.8-Max 权重规格补全**——`Qwen/Qwen3.8-2.4T-A95B` text-only / thinking required-on / 512 专家（10 routed + 1 shared）/ 定制 `qwen3.8-max` license；**GPT-5.6 Sol Ultrafast（Cerebras 08-14）**——官方合作加速服务，chart 14× / end-to-end 5.6×（非新模型）；**DeepSeek V4-Pro 官方 GA 确认（08-12/13，Reuters 08-13 报道 API 定价显著高于 V4 Flash）**）
> 交叉观察：**后训练 Scaling 成为新的竞争前沿**——GLM-5.3 首次把"同基座、纯后训练提智"作为旗舰发布叙事（智能上界 + 网络安全能力"涌现"），与 Kimi K3（2.5× scaling efficiency）、MiniMax M3（cowork/computer use）、GPT-OSS 时代的 reasoning_effort 相呼应，**"预训练基座 + 长程 RL 环境"成为差异化主轴**；**中国开源旗舰"周更"节奏**——两周内 Kimi K3 权重（07-27）、DeepSeek V4-Pro GA（08-12）、Qwen3.8-Max 2.4T 权重（08-12/13）+ Qwen3.8-27B（08-14）、GLM-5.3（08-14）连续放量（IT之家："国产旗舰月月有新版本"）；**开放权重"能力/许可证双轨"分化出现**——Qwen 首次把 Max 级权重与 27B 权重明确分层（定制 license + text-only + forced thinking vs Apache 2.0 + native vision），"开放权重 ≠ 全能力开放"成为新常态；**Meta 仍是唯一持续失约方（第 4 天）**——与 DeepSeek/Qwen/GLM 的"承诺→兑现"信用形成对照。

---

## 1. DeepSeek（深度求索）

### 1.1 DeepSeek V4 技术报告（继承 08-14，本轮补 API 定价口径）
...
> 今日核实：V4-Pro 官方 GA（08-12）获 Reuters 08-13 报道确认——API 定价显著高于 V4 Flash 若干倍（旗舰 vs 性价比分层明确）；OpenRouter `deepseek-v4-pro-0813`（$0.435/$0.87 每 M tokens）与 V4 Flash 均 MIT 开放权重，V4 家族全线开源。

### 1.2 V4-Flash Agent 能力（继承 08-14，保留）
...
### 2.1 GPT-5.6 System Card——GPT-Red 自动红队更新（继承 08-14）
...
### 2.2 GPT-5.6 Sol Ultrafast——Cerebras 加速服务（今日新增核实，08-14）
...
### 3.1 Llama 4 405B 开放权重——持续未兑现（第 4 天，08-15 核实）

> ⚠️ **验收结论延续（08-15）：** "Meta Llama 4 405B 开放权重"仍无发布实据——仅 NeuralStack 07-28 同一条预告反复出现（405B、原生多模态 text/image/audio、15T tokens 含 2.4T 图文对、单 H100 32 tok/s）；llama.com 目录仍仅 Llama 4 Scout/Maverick（2025-04 时代）。本轮注意到一篇 04-24 低可信度文章（annlive.com）宣称 "Llama 4 405B 已发布"，但与 mungomash（08-05 追踪"2025-04 后无新 Llama 发布"）、codersera（05-01）等一致性证据矛盾，判定为内容农场文章不采信。实际重大事件仍为 08-10 开放权重战略转向（开源 30B Muse Glimmer：Apache 2.0、128K ctx、由 Muse Spark 蒸馏 + 承诺数周内开源 Muse Spark 1.2 权重）。

### 3.2 Muse 系列（继承 08-14，保留）
...
### 4.1 Gemini 3.7 Flash GA（继承 08-14）
...
### 5.1 Claude Sonnet 5 / Opus 5 System Cards（继承 08-14）
...
> 今日核实：Claude Fable 5 为当前 frontier（Mythos Preview System Card 04-07 仍为最先进闭源前沿卡）；Fable 5.1 仍无官方公告（08-15 复核，仅两条 X 泄漏与 geeky-gadgets 07-28 "8 月 beta 发布"预告，$10/$50 为传闻）——观察项。

---

## 6. Mistral

### 6.1 Shieldstral 安全分类器（继承 08-14）
...
### 6.2 欧洲主权 AI 基础设施 & 机器人方向（继承 08-14，保留）
...
### 7.1 Qwen3.8-27B 正式开源（今日最大新增，08-14）
...
> 今日核实（关键对比）：**Qwen3.8 开放权重出现"能力/许可证双轨"**——27B 为 Apache 2.0 + 原生多模态（image+video）+ thinking 可调；2.4T 旗舰权重则为定制 `qwen3.8-max` license + **text-only（明确声明不支持多模态输入）+ thinking 强制开启**。Reddit r/LocalLLaMA 称 27B 为"beloved Qwen 的 renewal，unmatched intelligence density"，是 GPT-OSS 时代以来首个原生支持 low/medium reasoning 档位的本地模型。

### 7.2 Qwen3.8-Max 权重规格补全（08-12/13 已确认，本轮补规格）

| 项目 | 内容 |
|------|------|
| **中文标题** | Qwen3.8-Max 开源权重规格补全——text-only + 定制 license |
| **英文标题** | Qwen3.8-Max open weights spec completion |
| **发布机构** | Alibaba Qwen |
| **模型系列** | **Qwen3.8-2.4T-A95B**（2.4T 总参 Sparse MoE / ~95B active / 512 专家：10 routed + 1 shared）；Qwen3.5-family hybrid 架构（Gated DeltaNet + MoE + Gated Attention） |
| **发布日期** | API GA：2026-08-03；**权重：2026-08-12/13 上架 HF/ModelScope** |
| **核心创新** | **text-only 权重**（与多模态 API 明确区分）；**thinking required-on**（不可关闭）；native ctx **262,144**（~1.01M extensible）；**license `qwen3.8-max`**（定制，大客户 revenue share 条款——门槛与比例均未公开）；4-bit 量化约 1.2TB VRAM；vLLM/SGLang/TokenSpeed/Transformers 支持 |
| **论文** | https://qwen.ai/blog?id=qwen3.8 |

### 7.3 其他 Qwen 条目（继承 08-14，保留）
...
### 8.1 Phi-5 仍无官方技术报告（继承 08-14）

> 截至 08-15 **仍无 Phi-5 官方技术报告**；MSR 最新技术报告仍为 Phi-4-reasoning-vision-15B（2026-03）；16B/MMLU 86.7% 报道仍为 single-source（GogoAI），未确认。Phi-4 / Phi-4-mini / Phi-4-multimodal 仍为开放模型家族最新（MIT license）。

### 8.2 Phi Silica Platform Card（继承 08-14）
...
| **核心创新** | IFP（Instruction-Following Pruning）稀疏化；端侧/云侧 PCC 双轨；**与 Google 合作构建** |
| **论文** | Apple Foundation Models 3 Technical Report（2026-06-08） |

> 今日核实：正式发布承诺 "later this summer"（截至 08-15 仍在窗口内）未兑现；观察项。

---

## 10. NVIDIA

### 10.1 Nemotron 3.5 Lightning（继承 08-14）
...
| **核心创新** | 面向 **always-on agents**；**speculative decoding**；NVFP4 / BF16 checkpoints；最高 **4× 输出速度**；NeMo Switchyard 路由；开源 |
...
### 10.2 Nemotron 3 家族（继承 08-14，保留）
...
### 11.1 Grok 4.6 Model Card（继承 08-14）
...
| **核心创新** | **text+image 输入 / text-only 输出**；与 **Cursor** 合作开发；500K ctx；$2/$0.50/$6（<200k）与 $4/$1/$12（>200k）每 M tokens；reasoning 四档 low/medium/high/xhigh |
| **论文** | https://media.x.ai/v1/website/card-7f81d41b.pdf |

> 今日核实：Grok 4.7（2.1T）计划 3-4 周后、Grok 5 年内——仍为传闻。

---

## 12. Amazon

### 12.1 Nova 2 技术报告（继承 08-14）
...
### 13.2 Doubao 应用规模 & 其他（继承 08-14，保留）

- **Doubao**：155M 周活、全球第 4 大 GenAI 应用、春节峰值约 145M DAU；旗舰 **Doubao Seed 2.0 Pro**（2026-02-14：text / native video / 全双工语音多模态）。
- **>5T/10T 参数新模型训练传闻**（08-06/07）未发布；**Seed2.1 Pro + Turbo**（07）Agent/代码工程 SOTA。

---

## 14. Zhipu（智谱）

### 14.1 GLM-5.3 正式发布（今日最大新增，08-14）
...
| **评测** | Terminal-Bench 3.0 **4.6→28.3**（开源第一）；DeepSWE v1.1 **46.2→66.9**；Agents' Last Exam **23.8→28.5**（CLI 开源第一）；GDPval-AA v2 **1,769**；Z.ai Code Bench High 档 **31.4%**（超 Opus 4.8 最高档 29.5%，且每任务约 5 万 tokens vs Opus 4.8 约 12 万）；网络安全白盒代码审查/漏洞发现**持平 Mythos 5**，真实代码库累计识别 **2436** 个安全漏洞；"编程与智能体能力接近 Claude Fable 5" |
| **论文** | https://bigmodel.cn/（官方公告）；IT之家：https://view.inews.qq.com/a/20260814A084MO00 |

> ⚠️ **修正 08-14 记录：** 08-14 digest 曾记 "GLM-5.3 传闻（>1T，新浪财经 07-20 + JPMorgan 8 月口径）未发布，不入正式条目"——今日核实 GLM-5.3 **已于 08-14 正式发布**，且参数量为 **743B（与 5.2 同基座）**，此前 ">1T" 传闻口径错误；GLM-5.5 仍无消息。官方口径强调安全能力（漏洞发现）为后训练"涌现"，需第三方独立复测验证。

### 14.2 GLM-5.2（继承 08-14，已被 5.3 取代为发布主线）
...
| **核心创新** | 93 层（69 KDA + 24 Gated MLA）；896 路由专家（激活 16）；**Kimi Delta Attention** + **Attention Residuals** + **Stable LatentMoE**；MoonViT-V2；MXFP4/8；1M ctx；原生视觉；相对 K2 约 **2.5× scaling efficiency**；WebDev Arena #1 |
| **论文** | https://arxiv.org/abs/2607.24653 |

> 今日核实：Kimi K4 训练中（The Information 07-28/29）未发布；K3 仍落后 Claude Fable 5 / GPT-5.6 Sol 的闭源前沿。

---

## 16. InternLM（上海 AI 实验室）

### 16.1 InternGeometry（继承 08-14，ICLR 2026）
...
| **核心创新** | **CBRL（Complexity-Boosted RL）**；符号引擎交互；IMO 2000-2024 几何题 **44/50**（金牌平均 40.9）；仅 13K 训练样例 |
| **论文** | ICLR 2026（InternGeometry） |

### 16.2 家族状态（继承 08-14，保留）

- **InternLM3-8B-Instruct**（2026-01-15，4T tokens，训练成本 −75%）；**InternThinker**（2025-11-25）；**Intern-S2-Preview**（35B 07-17 / 397B 07-18，Apache-2.0）。
...
### 17.1 Baichuan-M4 与清华联合（继承 08-14）
...
| **发布机构** | 百川智能 + 清华大学 |
| **模型系列** | **Baichuan-M4**（临床级医疗 Agent）；Baichuan-M3（2026-01，235B）；Baichuan-M2（08-11，32B 开源，HealthBench 60.1） |
...
> 今日核实：战略全面转向医疗垂直；无 8 月新通用模型。

---

## 18. StepFun（阶跃星辰）

### 18.1 Step 3（继承 08-14，规格以官方 321B/38B 为准）
...
> 规格备忘：以官方 **321B 总参 / 38B 激活** 口径为准。Step3-VL-10B（2026-01，PaCoRe，arXiv:2601.09668）；Step 3.7 Flash（~2026-03，196B+1.8B/11B，400 tok/s，single-source）；Step 4 训练（2026-02 宣布）为下一观察点。
...
### 20.1 M3 完整规格（继承 08-14，仍为国内首个开放世界旗舰）
...
| **评测** | **BrowseComp 83.5**（超 Opus 4.7 的 79.3）；**TrainBench 37.1** 第三（仅次 Opus 4.7 42.4 / GPT-5.5 39.3）；BenchLM 68.6 |
...
- **后训练 Scaling 成为新的竞争前沿**：GLM-5.3 首次把"**同基座、纯后训练提智**"作为旗舰发布叙事（数十倍长程环境 + 超长后训练时间拉高智能上界），与 Kimi K3（2.5× scaling efficiency）、MiniMax M3（cowork/computer use）、GPT-OSS 时代的 reasoning_effort 共同指向——**预训练基座 + 长程 RL 环境（而非换基座）成为差异化主轴**；智谱自述"可能还远未开发出这个基座的智能上界"，为"基座即资产、后训练即杠杆"的行业共识再添注脚。
- **中国开源旗舰"周更"节奏**：8 月两周内 Kimi K3 权重（07-27）、DeepSeek V4-Pro GA（08-12）、Qwen3.8-Max 2.4T 权重（08-12/13）+ Qwen3.8-27B（08-14）、GLM-5.3（08-14）连续放量（IT之家："国产旗舰月月有新版本"）——开源前沿能力密度与兑现速度均在提升。
- **开放权重"能力/许可证双轨"分化出现**：Qwen3.8 首次出现同代权重的明确分层——2.4T 旗舰（定制 license + text-only + thinking 强制 + 大客户 revenue share 条款）vs 27B（Apache 2.0 + 原生多模态 + thinking 可调）；"开放权重 ≠ 全能力开放"，自托管者可能被迫选更小模型。对照 GLM-5.3 承诺两周后开源、DeepSeek V4 全系 MIT——中国开源阵营的许可策略开始分化（permissive vs 定制限制）。
- **Meta Llama 4 405B 持续未兑现（第 4 天）**：与 DeepSeek V4 Pro GA + MIT 权重、Qwen3.8 双权重兑现、GLM-5.3 开源承诺形成对照，Meta 成唯一持续失约方；本轮另剔除一条低可信度 "04-24 已发布" 内容农场文章（annlive.com）。
- **Agent/安全能力叙事延续**：GLM-5.3 网络安全"涌现"（2436 漏洞、持平 Mythos 5）接棒 MiniMax M3（计算机自主复现 ICLR 论文）、Qwen3.8-27B（OSWorld 84.3）——"能挖洞、能干活、能写码"成为开源旗舰标配话术。
- **传闻需谨慎（未确认不入正式条目）**：Meta Llama 4 405B（未兑现）、GLM-5.5（无消息）、Grok 4.7（2.1T）/Grok 5、Kimi K4、MiniMax M4（H2 2026）、Phi-5（single-source）、Fable 5.1（无官方公告）、InternLM4、字节 >5T/10T 新模型、Mistral 夏季"大而稀疏"开放权重、Astra"GPT-6"命名。
