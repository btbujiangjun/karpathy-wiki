---
title: "大模型技术报告摘要 — LLM Tech Report Digest (2026-07-04)"
type: synthesis
created: 2026-07-04
updated: 2026-07-04
sources: [web-search]
...
> 覆盖 20 家 AI 机构最新发布的大模型技术报告（Tech Report / System Card），持续更新。本文档为 2026-07-04 综合版。

## 目录

1. [DeepSeek — DeepSeek-V3 / R1 / V3.2](#1-deepseek)
2. [OpenAI — GPT-5 / o3 / o4-mini](#2-openai)
3. [Meta AI — Llama 4](#3-meta-ai)
4. [Google DeepMind — Gemini 2.5](#4-google-deepmind)
5. [Anthropic — Claude Opus 4 / Sonnet 4](#5-anthropic)
6. [Mistral AI — Mistral Large 3 / Ministral 3](#6-mistral-ai)
7. [Qwen (Alibaba) — Qwen3](#7-qwen-alibaba)
...
| **输入模态** | 多语言文本 + 图像 |
| **输出模态** | 多语言文本 + 代码 |
| **主要创新** | 原生多模态（早期融合，非 adapter）；Scout 单 H100 int4 可部署；Maverick FP8 单 DGX 节点可部署；10M 超长上下文 (Scout) |
...
| 项目 | 内容 |
|------|------|
| **中文标题** | Phi-4 技术报告 / Phi-4-reasoning 技术报告 / Phi-4-reasoning-vision 技术报告 |
...
| **上下文长度** | 256K tokens |
| **K2.5 新增** | 在 K2-Base 上继续预训练 ~15T 混合视觉+文本 tokens；MoonViT 视觉编码器 (400M 参数) |
| **主要创新** | Agentic 能力 SoTA；MLA (Multi-head Latent Attention)；原生多模态 Agent（K2.5）；Thinking + Instant 双模式；开源 |
...
| 维度 | 趋势 |
|------|------|
| **MoE** | 几乎所有旗舰模型（DeepSeek-V3/R1/V3.2, Llama 4, Qwen3-235B, Mistral Large 3, Nemotron 3, GLM-5, Kimi K2/K2.5, Step-2）均采用 MoE。激活参数比率从 5%~10% 不等 |
| **Hybrid Mamba-Attention** | NVIDIA Nemotron 3 系列率先落地混合 Mamba-Transformer 架构 |
| **MLA (Multi-head Latent Attention)** | DeepSeek 自研 MLA 被 Kimi K2/K2.5 采纳 |
| **DeepSeek Sparse Attention (DSA)** | DeepSeek V3.2 提出，被 GLM-5 集成 |
| **原生多模态** | Llama 4 早期融合（非 adapter）；Kimi K2.5 原生融合视觉 |
...
| 模型 | 策略 |
|------|------|
| DeepSeek-R1 / V3.2 | CoT RL, thinking mode |
