...
created: 2026-08-06
updated: 2026-08-06
sources: [tech-report-digest-2026-08-05.md]
...
> 更新日期：2026-08-06（每日更新；今日重点：Qwen3.8-Max 开源权重窗口临近（08-03 官方博客确认"下周"开源，预计 08-10 前后，仍缺具体日期/license/model card）、xAI Grok 4.6 发布倒计时（明日 08-07，1.5T 参数、升级 SFT/RL；Grok 4.7 2.1T 随后）、OpenAI GPT-5.7 泄漏复核（8 月发布、新 pretraining foundation ~10T tokens；GPT-6 推迟至 9 月）、Apple Siri AI 进入 iOS 27 消费者测试版（TechCrunch 08-03，与 Google Gemini 合作训练 Apple Foundation Models）、Moonshot Kimi K4 训练传闻（寻求更多 Blackwell 芯片）、Microsoft Phi-5 仍无官方技术报告（仅 Inference Index 目录条目）；NVIDIA Nemotron 3 Ultra 技术报告细节复核、Mistral Shieldstral 复核，均无变更）
...
> ⚠️ 传闻更新（今日复核）：WinCentral 2026-07-30 报道 **GPT-5.7**（8 月发布，新的 pretraining foundation，约 10T tokens 规模训练，更强推理与 agent 能力），GPT-6 或推迟至 9 月；另据 The Information 报道 OpenAI 内部代号 **Astra** 的新模型家族（面向长期运行任务，已解决 10 道长期未解数学题），命名可能在 GPT-5.7 / GPT-6 之间未定。截至 08-06 仍为记者爆料，未获官方确认，不写入正式条目。
...
> 今日核实：搜索未发现 8 月新报告（仅旧 Llama 3 MODEL_CARD 命中）；无 Llama 5 新消息；Muse 系列为当前主线。
...
> 今日核实：无 8 月新报告；最新官方 System Card 为 Claude Opus 5（07-24）。

---

## 6. Mistral

### 6.1 Shieldstral（08-05 已收录；今日复核细节）
...
### 7.1 Qwen3.8-Max（08-04 已收录；今日更新开源窗口状态）
...
> 今日状态：开源权重窗口临近——08-03 官方承诺"下周"开源，预计 **08-10 前后**放 HF/ModelScope；截至 08-06 仍无具体日期 + license + model card（noze.it 评论：Qwen3.8-27B 才是普通企业可自托管的关键 checkpoint；对照 Moonshot Kimi K3 的明确权重日期 07-27）。
...
> ⚠️ 传闻更新（今日核实）：**Phi-5** 截至 08-06 仍无官方技术报告——唯一新增为 Inference Index 目录条目（2026-01-08，128K ctx，小模型 champion）；此前的 16B / MMLU 86.7% 报道为 single-source（GogoAI），官方 MSR 页面仍以 Phi-4-reasoning-vision-15B 为最新技术报告，**未确认，不写入正式条目**。
...
| **模型系列** | AFM 3 Core（3B 端侧）/ Core Advanced（20B 端侧稀疏，IFP 激活 1–4B）/ AFM 3 Cloud（PCC）/ AFM 3 Cloud Pro（PCC on Google Cloud NVIDIA GPU） |
| **发布日期** | 2026-06-08（WWDC26） |
| **核心创新** | IFP（Instruction-Following Pruning）+ 大量 always-active shared experts；AFM 3 Cloud Pro 首次把 PCC 扩展到 Google Cloud NVIDIA GPU |
| **论文** | https://machinelearning.apple.com/research/introducing-third-generation-of-apple-foundation-models |

> 今日新增核实：TechCrunch 2026-08-03（"Apple finally fixed Siri"）——**Siri AI 已进入 iOS 27 消费者测试版**（beta，2026-07 起）；Apple Foundation Models 训练借助 **Google Gemini 合作**（与 AFM 3 发布的"与 Google 合作定制"一致）；AFM 3 正式技术报告仍待发布（"later this summer" 预期未兑现）。
...
### 10.2 Nemotron 3 Ultra Technical Report（08-02/03 已收录；今日复核细节）
...
> Nemotron 3 三档（Nano 08-04 / Super 08-05 / Ultra 08-02-03）技术报告现已全部收录，构成完整家族：混合 Mamba-Attention + LatentMoE + NVFP4 预训练 + 1M ctx + 开源。今日复核：Ultra 报告细节（06-09，550B/55B，20T tokens）无变更。

---

## 11. xAI

### 11.1 Grok 4.5（已收录；今日核实 4.6 / 4.7 时间表）
...
> ⚠️ 传闻更新（今日复核）：Musk（2026-07-28；Economic Times 07-29 报道）确认 **Grok 4.6 预计 2026-08-07（明日）发布**（1.5T 参数 V9 基座、大幅升级 SFT/RL），**Grok 4.7**（2.1T）随后数周发布。仍为预告/口头时间表，未正式发布，不作为正式条目；**08-07 为观察日**。
...
> ⚠️ **GLM-5.5** 截至 08-06 仍未发布：JPMorgan 研报（Reuters/CGTN 转载）称可能 2026-08 发布，1T+ 参数（基于 GLM-5.2 744B 升级）、1M ctx。单源传闻，不写入正式条目；GLM-5.2 仍为当前确认旗舰。
...
> 今日新增传闻：Moonshot 寻求更多 NVIDIA **Blackwell** 芯片以训练 **Kimi K4**（AI Weekly 2026-07-28）——训练阶段传闻，未发布，不入正式条目；K3 技术报告（07-27）仍为最新。K3 权重已按承诺日期发布（对照 Qwen3.8-Max 的"下周"）。
...
> 今日复核：HF `internlm/Intern-S2-Preview`（35B，2026-07-17）确认可下载；35B 与 397B 均为已发布规格。
...
- **Step 3.7 Flash**（当前旗舰，官网现列）：198B 总 / 11B 激活稀疏 MoE；原生多模态；256K ctx；low/medium/high 三级 reasoning level。
...
- **Qwen3.8-Max 开源窗口 = 本周最大观察点**：08-03 官方承诺"下周"开源（预计 08-10 前后），对照 Moonshot Kimi K3 已按期放权（07-27）——"承诺开源 + 给日期 + 给 license"正在成为 2T+ 开源旗舰的竞争标准，Qwen 缺最后三项细节受评论界质疑。
- **8 月上旬密集发布窗口**：明日（08-07）Grok 4.6 预告发布 + 传闻中 GPT-5.7（8 月）——若二者兑现，将与 Qwen3.8-Max 开源、Nemotron 3 家族、K3 构成 8 月第二波"旗舰对决"。
...
- **传闻需谨慎（未确认不入正式条目）**：GPT-5.7/Astra（The Information + WinCentral）、GLM-5.5（JPMorgan 8 月）、Phi-5（仅 Inference Index 目录条目）、Grok 4.6/4.7（Musk 口头时间表，08-07 观察）、Kimi K4（Blackwell 训练传闻）。
