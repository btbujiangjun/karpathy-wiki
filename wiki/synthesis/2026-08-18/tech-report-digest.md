---
title: LLM Tech Report Digest — 2026-08-18
type: synthesis
created: 2026-08-18
updated: 2026-08-18
sources: [web-search]
...
# LLM Tech Report Digest — 2026-08-18
...
| 最新动态 | V4-Pro GA（08-13）：Agent 能力大幅提升（HLE 60.0, TerminalBench 87.9, NL2Repo 61.5）；三档 Thinking Effort（low/high/max）；Responses API + Codex 适配 |
...
| 发布日期 | 2026-08（GPT-5.6 System Card） |
| 开源状态 | ❌ 闭源 |
...
| 最新动态 | Astra 定名（08-01 官方博客）——"next major model"；内部 Astra 证明 10 个数学/理论计算机科学定理（$2,000 API token）；Astra 网络安全能力触发 Critical 评估暂停（08-07 Axios 报道）；GPT-5.6 仍为最新公开旗舰 |
...
| 最新动态 | Opus 5 现为最新旗舰；Claude Opus 4.6 MLE Bench 75.7%（最佳）；Opus 5 知识截止 2026-05；Mythos 5 在 CyberGym/ExploitBench 领先 |
...
| 项 | 值 |
|---|---|
| 最新旗舰 | Mistral Small 4 / Shieldstral / Ministral 3 |
| 发布日期 | 2026-03-16（Small 4）/ 2026-08-04（Shieldstral）/ 2026-01（Ministral 3） |
| 开源状态 | ✅ Apache 2.0 |
| 核心参数 | Small 4: 119B 总参 / 6B 激活（128 专家 MoE）；Shieldstral: 3B（安全分类器）；Ministral 3: 3B/8B/14B（Dense） |
| 上下文窗口 | 256K |
| 架构创新 | Small 4: 统一 Magistral(推理) + Pixtral(多模态) + Devstral(编码) 为单一模型；Shieldstral: 策略自适应多模态安全分类器（推理时自然语言策略，无需重训）；Ministral 3: Cascade Distillation 预训练方法 |
| 核心贡献 | Small 4: 40% 延迟降低 + 3× 吞吐提升；Shieldstral: 3B 匹配 20B 安全模型；Ministral 3: 高效 Dense 模型（3B/8B/14B） |
| 论文 | [Ministral 3](https://arxiv.org/abs/2601.08584) |
| 最新动态 | Shieldstral 1.0 3B（08-04）：Apache 2.0 多模态安全分类器；Small 4 首次统一推理+多模态+编码；夏季新"大而稀疏"MoE 预告 |
...
| 项 | 值 |
|---|---|
| 最新旗舰 | Qwen3.8-Max / Qwen3.8-27B |
| 发布日期 | 2026-08-03（Max）/ 2026-08-17（27B + Max 权重） |
| 开源状态 | ✅ Apache 2.0 |
| 核心参数 | Qwen3.8-Max: 2.4T 总参 / 95B 激活 MoE；Qwen3.8-27B: 27B Dense |
| 上下文窗口 | 1M（Max）/ 262K（27B，可扩展至 1M） |
| 架构创新 | Sparse MoE + 混合注意力（Hybrid Attention）；Qwen3.8-Max 首次 Qwen-Max 级开源权重；27B Dense 可量化至笔记本运行 |
...
| 最新动态 | Qwen3.8-27B 开源（08-17）：Apache 2.0，HF 2 天内 Top 5 最受欢迎模型；Max 权重同步开放；Qwen MM-Plugins 多模态 Agent 框架开源 |
...
| 核心贡献 | HealthBench 60.1（32B 超 120B）；延续医疗垂直战略 |
| 论文 | 未发布独立技术报告 |
| 最新动态 | 延续医疗垂直化路线；开源策略 |
...
| 项 | 值 |
|---|---|
| 最新旗舰 | Apple Foundation Model 3（AFM 3）家族 |
...
| 最新动态 | AFM 3 官方公告（06-08）；TR 承诺"summer"仍未兑现（截至 08-18）；5 模型家族（2 端侧 + 3 云端）|
...
| 最新动态 | Grok 4.6 GA（08-12）：Cursor + Grok Build 首发；Grok 4.7（2.1T）计划 3-4 周后；Self-harm 合规从 0.5% 退步至 3.7%（Model Card 自报） |
...
| 最新动态 | GLM-5.3 发布（08-14）：post-training scaling 驱动全部增益；网络安全能力涌现超出预期；权重计划两周后开源 |
...
| Apple AFM 3 TR "summer" | ❌ 未兑现（截至 08-18） | 06-08 公告后无技术报告 |
...
1. **GLM-5.3 post-training scaling 有效性验证**：同一基座模型，纯 post-training 带来 Terminal-Bench 4.6→28.3 跳升 + 网络安全能力涌现
2. **Kimi K3 KDA 架构**：首个大规模纯线性注意力前沿模型（69/93 层 KDA），WebDev Arena 首次由开源模型登顶
3. **Apple AFM 3 Core Advanced**：首个端侧 20B Sparse 模型（IFP 剪枝），1-4B 激活
4. **InternLM Mobius 架构**：知识-推理解耦（共享 Memory FFN + 多 Reasoner Self-Attn），35B 达 4× 推理加速
5. **NVIDIA MOPD**：Multi-teacher On-Policy Distillation，10+ 专用教师模型指导学生训练
6. **Step 3.7 Advisor Mode**：小执行器 + 大顾问模型，97% Opus 4.6 性能 @ 1/9 成本

---

*Generated 2026-08-18. Source: Web search results. Cross-referenced with wiki/synthesis/2026-08-17/tech-report-digest.md.*
