...
>
> **Dedup discipline**: today's sibling digests (arxiv-ai-search / arxiv-daily / arxiv-paper-check / game-rl-daily, all 2026-09-25) already committed. **22 featured fresh-window IDs are exclusive to this digest**: grep-verified absent from all 4 siblings' claimed sets AND absent from `wiki/` at write time. **6 featured entries overlap with same-day sibling claims** (R3 slate − all 3 search siblings, C2 ScalarLens − ai-search §1.2, L1 PoEM / A2 AdvRole / W1 WROP / W2 HelloWorld − game-rl-daily) — marked `⚠️dup`, cross-referenced §5, not silently re-featured. **2 venue-sweep records** (C1 HOBA、L3 Flexibility Trap) pre-date this digest (08-26 daily / 07-30→09-05 digests) — kept as KDD/ICML venue confirmation, not claimed as exclusive.
